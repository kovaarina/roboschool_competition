...
class CoRLRewards:
    ...
    def _reward_feet_clearance_cmd_linear(self):
        ...
        v = torch.norm(self.env.commands[:, :2], dim=1)
        v_on   = self.env.cfg.rewards.stand_vel_on
        v_full = self.env.cfg.rewards.stand_vel_full

        x = (v - v_on) / (v_full - v_on)
        x = torch.clamp(x, 0.0, 1.0)
        w_walk = _smoothstep(x)

        reward = reward * w_walk
    ...
    def _reward_stand_still(self):
        ...
        v = torch.norm(self.env.commands[:, :2], dim=1)
        v_off  = self.env.cfg.rewards.stand_vel_off
        v_full = self.env.cfg.rewards.stand_vel_full

        x = (v - v_off) / (v_full - v_off)
        x = torch.clamp(x, 0.0, 1.0)
        w_walk = _smoothstep(x)
        w_stand = 1.0 - w_walk

        # pos_cost = torch.sum(torch.abs(self.env.dof_pos - self.env.default_dof_pos), dim=1)
        pos_cost = torch.sum(torch.abs(self.env.actions), dim=1)
        vel_cost = torch.sum(torch.square(self.env.dof_vel), dim=1)

        return w_stand * (0.2 * pos_cost + 0.5 * vel_cost) # * (torch.norm(self.env.commands[:, :2], dim=1) < 0.2)
